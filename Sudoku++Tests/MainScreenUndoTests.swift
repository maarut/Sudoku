...
class MainScreenUndoTests: XCTestCase {
    ...
    func testUndoOfAutoGeneratedPencilMarks()
    {
        let cell = Cell()
        cell.pencilMarks = [1,2,3]
        sudokuBoard.stub(.dimensionality, andReturn: 1)
        sudokuBoard.stub(.cellAtIndex, andReturn: cell as Cell?)
        viewModel.fillInPencilMarks()
        mockViewModelDelegate.resetMock()
        mockViewModelDelegate.expect(.showPencilMarks, withArgs:
            AnyEquatable(base: [Int]()),
            AnyEquatable(base: nil, equatableImpl: { _ in true }) )
        viewModel.undo()
        mockViewModelDelegate.verify()
    }
}
